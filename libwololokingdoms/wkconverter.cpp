...
int WKConverter::run() {
  ...
  if (settings.patch < 0) {
    ...
    if (settings.copyCustomMaps) {
      ...
      convert_maps(settings.hdPath / "resources" / "_common" /
                     "random-map-scripts",
                 installMapDir, newTerrainFiles);
    } else {
    ...
    if (settings.copyMaps) {
      convert_maps(resourceDir / "Script.Rm", installMapDir, newTerrainFiles, true);
    } else {
